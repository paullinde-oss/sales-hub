...
const APP_VERSION = "v4.9 — Jun 2026";
...
const Icon = ({ name, size=18, color="currentColor" }) => {
  const paths = {
    ...
    dims: <><line x1="3" y1="21" x2="21" y2="21" stroke={color} strokeWidth="1.5"/><line x1="3" y1="21" x2="3" y2="3" stroke={color} strokeWidth="1.5"/><line x1="3" y1="17" x2="7" y2="17" stroke={color} strokeWidth="1.5"/><line x1="3" y1="13" x2="9" y2="13" stroke={color} strokeWidth="1.5"/><line x1="3" y1="9" x2="7" y2="9" stroke={color} strokeWidth="1.5"/><line x1="3" y1="5" x2="9" y2="5" stroke={color} strokeWidth="1.5"/><path d="M7 21L21 7" stroke={color} strokeWidth="1.5"/></>,
...
class TabErrorBoundary extends Component {
  ...
  componentDidUpdate(prev){ if(prev.tabKey!==this.props.tabKey && this.state.hasError){ this.setState({hasError:false,msg:""}); } }
  render(){
    if(this.state.hasError){
      return (
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:12,padding:24,textAlign:"center"}}>
          <div style={{fontSize:14,color:"#e8472c",fontWeight:600}}>Something went wrong loading this tab</div>
          <div style={{fontSize:11,color:"#888",maxWidth:420,fontFamily:"monospace"}}>{this.state.msg}</div>
          <button onClick={()=>this.setState({hasError:false,msg:""})}
            style={{background:"#c8a96e",border:"none",color:"#0a0a0a",padding:"8px 18px",fontSize:11,fontWeight:600,cursor:"pointer",letterSpacing:".04em"}}>
            Try Again
          </button>
        </div>
      );
...
export default function SalesHub() {
  ...
  function setCurrentProducts(updater) {
    if (productCurrency === "CAD") {
      setProductsCAD(prev => {
        ...
        setDoc(doc(db, "config", "productsCAD"), {items: next}).catch(console.error);
        return next;
      });
    } else {
      setProductsUSD(updater);
    }
  }
  function setCurrentCategories(updater) {
    setCategories(prev => {
      const next = typeof updater === 'function' ? updater(prev) : updater;
  ...
  function saveQuote(q) {
    ...
    // ── Sync to Lead Tracking if isNewLead ────────────────────────────────────
    if (saved.isNewLead) {
      const total = (saved.lineItems||[]).reduce((s,li)=>(parseFloat(li.unitPrice)||0)*(parseInt(li.qty)||0)+s, 0);
      const skus  = (saved.lineItems||[]).map(li=>li.sku||"").filter(Boolean).join(", ");
      const yr    = new Date().getFullYear();
      const normStatus = s => s==='won'?'won':s==='lost'?'lost':s==='cold'?'cold':'inprogress';
      setLeads(prev => {
        // Check if lead already exists for this quote number
        const existing = prev.findIndex(l => l.quoteNumber === saved.quoteNum);
        const entry = {
          id:              existing >= 0 ? prev[existing].id : Date.now(),
          company:         saved.company || saved.name || "",
          contact:         saved.name || "",
          type:            saved.leadType || "End User",
          leadSource:      saved.leadSource || "Ads/Website",
          potentialValue:  total,
          status:          normStatus(saved.quoteStatus),
          lastContact:     saved.savedDate,
          productsQuoted:  skus,
          notes:           saved.internalNotes || saved.notes || "",
          quoteNumber:     saved.quoteNum,
          year:            yr,
          isLead:          true,
        };
        if (existing >= 0) {
  ...
  function duplicateQuote(q) {
    ...
    const duped = {
      ...
      validFor:      parseInt(q.validFor) || 30,
      quoteStatus:   q.quoteStatus || "inprogress",
      followUps:     [],  // fresh follow-up history on duplicate
      saved:         false,
      savedBy:       "",
      savedDate:     "",
      lineItems:     newItems,
        ...
        {!activeQuote
          ? <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:16}}>
              ...
              <div style={{textAlign:"center"}}>
                ...
                <div style={{fontSize:11,color:T.muted||"#999"}}>Select one from the list or create a new one</div>
              </div>
...
function QuoteForm({quote,setQuote,productsCAD,productsUSD,onSave,onEdit,onEmail,onPDF,onClose,onNewQuote,isMobile,exchangeRate=0.73,T}) {
  ...
  function flashPrice(id) {
    ...
    setTimeout(() => setPriceFlash(f => ({...f, [id]: false})), 1400);
  }
  ...
  return (
    ...
    <div>
              ...
              <td>
                {ro
                  ...
                  : <div style={{position:"relative"}}>
                      ...
                      {priceFlash[li.id]&&(
                        <div style={{position:"absolute",right:-72,top:-1,background:"#1a3a1a",border:"1px solid #2a6a2a",color:"#5ace5a",fontSize:9,padding:"2px 6px",borderRadius:2,whiteSpace:"nowrap",pointerEvents:"none",zIndex:10,animation:"fadeOut 1.4s forwards"}}>
                          ✓ Updated
                        </div>
                      )}
                    </div>
      ...
      {/* Coverage Totals — Turbidity Curtains, Straw Wattles, ECB/TRM */}
      {(()=>{
        const items = quote.lineItems || [];

        // Turbidity Curtains: SKU contains "TBC" — each section = 50ft / 15m
        const curtainQty = items.filter(li => /TBC/i.test(li.sku||'')).reduce((s,li)=>s+(parseInt(li.qty)||0),0);

        // Straw Wattles: STW0925/RDSTW0925/BSTW0925/AEW0925 = 25ft; STW1220/RDSTW1220/BSTW1220/AEW1220 = 20ft
        const wattle25Qty = items.filter(li => /0925/i.test(li.sku||'')).reduce((s,li)=>s+(parseInt(li.qty)||0),0);
        const wattle20Qty = items.filter(li => /1220/i.test(li.sku||'')).reduce((s,li)=>s+(parseInt(li.qty)||0),0);
        const totalWattleFt = wattle25Qty * 25 + wattle20Qty * 20;

        // ECB/TRM: size by SKU suffix
        // 08 = 100sy/83m²  16 = 200sy/167m²  08JR = 500sy/418m²  16JR = 1000sy/836m²
        function ecbSize(sku) {
          if (/16JR/i.test(sku)) return {sy:1000,m2:836};
          if (/08JR/i.test(sku)) return {sy:500, m2:418};
          if (/16/i.test(sku))   return {sy:200, m2:167};
          if (/08/i.test(sku))   return {sy:100, m2:83};
          return null;
        }
        let totalSY = 0, totalM2 = 0;
        items.forEach(li => {
          const sku = li.sku || '';
          if (!/^BMP-/i.test(sku)) return; // only BMP- prefix products
          const size = ecbSize(sku);
          if (!size) return;
          const qty = parseInt(li.qty) || 0;
          totalSY += qty * size.sy;
          totalM2 += qty * size.m2;
        });
        const totalSF = Math.round(totalSY * 9);

        const hasCurtains = curtainQty > 0;
        const hasWattles  = wattle25Qty > 0 || wattle20Qty > 0;
        const hasECB      = totalSY > 0;
        if (!hasCurtains && !hasWattles && !hasECB) return null;

        return (
          <div style={{border:`1px solid ${T.border}`,marginBottom:8,overflow:"hidden"}}>
            <div style={{fontSize:9,textTransform:"uppercase",letterSpacing:".12em",color:T.muted,padding:"5px 10px",background:T.tableHead,borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:6}}>
              <span>📐</span> Coverage Totals
            </div>
            <div style={{display:"flex",flexWrap:"wrap",gap:0}}>
              {hasCurtains && (
                <div style={{padding:"8px 14px",borderRight:`1px solid ${T.border}`,minWidth:160}}>
                  <div style={{fontSize:9,color:T.muted,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>Turbidity Curtain</div>
                  <div style={{fontSize:13,fontWeight:600,color:T.subtext,fontVariantNumeric:"tabular-nums"}}>
                    {(curtainQty*50).toLocaleString()} ft &nbsp;<span style={{color:T.muted,fontWeight:400,fontSize:11}}>({(curtainQty*15).toLocaleString()} m)</span>
                  </div>
                  <div style={{fontSize:10,color:T.muted,marginTop:2}}>{curtainQty} section{curtainQty!==1?"s":""} × 50 ft each</div>
                </div>
              )}
              {hasWattles && (
                <div style={{padding:"8px 14px",borderRight:`1px solid ${T.border}`,minWidth:160}}>
                  <div style={{fontSize:9,color:T.muted,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>Straw Wattles</div>
                  <div style={{fontSize:13,fontWeight:600,color:T.subtext,fontVariantNumeric:"tabular-nums"}}>
                    {totalWattleFt.toLocaleString()} ft
                  </div>
                  {wattle25Qty>0 && <div style={{fontSize:10,color:T.muted,marginTop:2}}>{wattle25Qty} × 25 ft</div>}
                  {wattle20Qty>0 && <div style={{fontSize:10,color:T.muted,marginTop:2}}>{wattle20Qty} × 20 ft</div>}
                </div>
              )}
              {hasECB && (
                <div style={{padding:"8px 14px",minWidth:220}}>
                  <div style={{fontSize:9,color:T.muted,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>ECB / TRM Coverage</div>
                  <div style={{display:"flex",gap:16}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:T.subtext,fontVariantNumeric:"tabular-nums"}}>{totalSY.toLocaleString()} sy</div>
                      <div style={{fontSize:9,color:T.muted,marginTop:1}}>Square yards</div>
                    </div>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:T.subtext,fontVariantNumeric:"tabular-nums"}}>{totalM2.toLocaleString()} m²</div>
                      <div style={{fontSize:9,color:T.muted,marginTop:1}}>Square metres</div>
                    </div>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:T.subtext,fontVariantNumeric:"tabular-nums"}}>{totalSF.toLocaleString()} sf</div>
                      <div style={{fontSize:9,color:T.muted,marginTop:1}}>Square feet</div>
                    </div>
                  </div>
                </div>
              )}
    ...
    (quote.lineItems||[]).forEach(li=>{
      ...
      Object.entries(STOCK_SHEETS).forEach(([key, sheet])=>{
        ...
        sheetData.rows.forEach(row=>{
          ...
          // Curtain: always show the row even if 0 — just don't add a redundant Calgary fallback card
          // Calgary Stock row above already shows any Calgary qty
...
function ProductEditRow({row,setRow,onSave,onCancel,availableSubCats=[],currentSubCat="",onSubCatChange}) {
  ...
  return (
    <tr style={{verticalAlign:"top"}}>
      <td style={{paddingTop:6}}><input {...f("sku")} style={{width:"100%",fontSize:11,height:26,fontFamily:"monospace",color:"#c8a96e"}}/></td>
      <td><input {...f("product")}/></td>
      <td style={{minWidth:220}}>
        <textarea value={row.description??""} onChange={e=>setRow(r=>({...r,description:e.target.value}))}
          style={{width:"100%",fontSize:11,minHeight:52,height:"auto",resize:"vertical",
            fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif",
            background:"var(--input-bg)",border:"1px solid var(--border-light)",
            color:"var(--text)",padding:"4px 6px",lineHeight:1.5,boxSizing:"border-box"}}
          rows={3}
...
function ProductsTab({products,setProducts,currency,setCurrency,search,setSearch,categories,setCategories,subCategories,setSubCategories,subCatDefs,setSubCatDefs,exchangeRate,setExchangeRate,T}) {
  const [editing,      setEditing]      = useState(null);
  const [sortF,        setSortF]        = useState(null);  // null = manual/array order (lets ↑↓ reorder work)
  const [sortD,        setSortD]        = useState(1);
  ...
  const [collapsed,    setCollapsed]    = useState(()=>new Set(["__all__"]));
  ...
  const [priceListModal, setPriceListModal] = useState(false); // Price List PDF modal
...
NEW-SKU-001,New Product Name,Full product description here,,6,48,$99.00,$94.00,,,$85.00,Misc. BMP Products`;
  ...
  // All unique category names — ordered by saved order (in subCatDefs.__catOrder__) or CATEGORY_ORDER default
  const allCategories = useMemo(()=> {
    try {
      const assigned = new Set(Object.values(categories||{}).filter(Boolean));
      const savedOrder = (subCatDefs||{}).__catOrder__;
      const baseOrder = Array.isArray(savedOrder) && savedOrder.length ? savedOrder : CATEGORY_ORDER;
      const ordered = baseOrder.filter(cat => assigned.has(cat));
      const extras = Array.from(assigned).filter(cat => !baseOrder.includes(cat)).sort();
      return [...ordered, ...extras, "Uncategorized"];
    } catch(e) { return ["Uncategorized"]; }
  }, [categories, subCatDefs]);

  // Move a category up/down — persists order to subCatDefs.__catOrder__
  function moveCategory(cat, dir) {
    const visible = allCategories.filter(c=>c!=="Uncategorized");
    const idx = visible.indexOf(cat);
    if (idx < 0) return;
    const newIdx = idx + dir;
    if (newIdx < 0 || newIdx >= visible.length) return;
    const reordered = [...visible];
    [reordered[idx], reordered[newIdx]] = [reordered[newIdx], reordered[idx]];
    setSubCatDefs(prev => ({...(prev||{}), __catOrder__: reordered}));
  }

  // Sub-categories per category: always show defined ones (in defined order), plus any inferred
  const subCatsByCat = useMemo(()=>{
    const result = {};
    // Start with defined order
    Object.keys(subCatDefs||{}).forEach(cat=>{
      if (cat==="__catOrder__") return;
      result[cat] = [...(subCatDefs[cat]||[])];
    });
    // Add any assigned-but-not-defined ones (appended, sorted)
    (products||[]).forEach(p=>{
  ...
  function sortProducts(arr) {
    if (!sortF) return [...arr]; // manual/array order — enables ↑↓ reordering
  ...
  function hs(f){
    if(sortF===f){
      if(sortD===1) setSortD(-1);
      else { setSortF(null); setSortD(1); } // third click returns to manual order
  ...
  function toggleCollapse(cat) {
    setCollapsed(prev => {
      const n = new Set(prev);
      if (n.has("__all__")) {
        // Default-collapsed mode: toggling opens it (add __open__ marker)
        if (n.has(`__open__${cat}`)) {
          n.delete(`__open__${cat}`); // re-collapse
        } else {
          n.add(`__open__${cat}`); // open it
        }
      } else {
        // Normal mode
  ...
  function moveSubCat(cat, sub, dir) { // dir: -1 up, +1 down
    setSubCatDefs(prev=>{
      const arr=[...(prev[cat]||[])];
  ...
  function moveProductInCat(sku, dir) {
    // Move a product up (-1) or down (+1) within its category/sub-category group
    // setProducts updater receives the FULL product list (from setCurrentProducts)
    const cat = (categories||{})[sku] || "Uncategorized";
    const sub = (subCategories||{})[sku] || null;
    setProducts(prev => {
      const arr = [...prev];
      // Get all products in same cat+sub in order
      const groupSkus = arr
        .filter(p => ((categories||{})[p.sku]||"Uncategorized")===cat && ((subCategories||{})[p.sku]||null)===sub)
        .map(p => p.sku);
      const groupIdx = groupSkus.indexOf(sku);
      if (groupIdx < 0) return prev;
      const newGroupIdx = groupIdx + dir;
      if (newGroupIdx < 0 || newGroupIdx >= groupSkus.length) return prev;
      const posA = arr.findIndex(p => p.sku === groupSkus[groupIdx]);
      const posB = arr.findIndex(p => p.sku === groupSkus[newGroupIdx]);
      if (posA < 0 || posB < 0) return prev;
      const newArr = [...arr];
      [newArr[posA], newArr[posB]] = [newArr[posB], newArr[posA]];
      return newArr;
    });
  }
  ...
  function copyProductTable(p) {
    const cols = [
      {h:"SKU",           v: p.sku},
      {h:"Description",   v: p.description||p.product},
      {h:"Price/Per",     v: parsePrice(p.price)>0 ? fmtCur(p.price) : null},
      {h:"Pallet Price",  v: parsePrice(p.palletPrice)>0 ? fmtCur(p.palletPrice) : null},
      {h:"Prepaid/Per",   v: parsePrice(p.prepaid)>0 ? fmtCur(p.prepaid) : null},
      {h:"Pre. Pallet",   v: parsePrice(p.prepaidPallet)>0 ? fmtCur(p.prepaidPallet) : null},
      {h:"Truck Price",   v: parsePrice(p.truckPrice)>0 ? fmtCur(p.truckPrice) : null},
    ].filter(c => c.v !== null);

    const headerRow = cols.map(c=>`<th style="background:#1a1a1a;color:#ffffff;padding:6px 10px;border:1px solid #444;font-family:Arial,sans-serif;font-size:11px;white-space:nowrap;text-align:left">${c.h}</th>`).join("");
    const dataRow   = cols.map(c=>`<td style="padding:6px 10px;border:1px solid #ccc;font-family:Arial,sans-serif;font-size:11px">${c.v}</td>`).join("");
    const html = `<table style="border-collapse:collapse;font-family:Arial,sans-serif"><thead><tr>${headerRow}</tr></thead><tbody><tr>${dataRow}</tr></tbody></table>`;
    const text = `${cols.map(c=>c.h).join("\t")}\n${cols.map(c=>c.v).join("\t")}`;

    const showCopied = ()=>{ setCopiedSku(p.sku); setTimeout(()=>setCopiedSku(null), 1500); };

    // Try modern clipboard API with HTML, fall back to writeText, then execCommand
    if (navigator.clipboard && window.ClipboardItem) {
      try {
        ...
        navigator.clipboard.write([item]).then(showCopied).catch(()=>{
          navigator.clipboard.writeText(text).then(showCopied).catch(()=>legacyCopy(html, text, showCopied));
        });
        return;
    ...
    if (navigator.clipboard?.writeText) {
      ...
      legacyCopy(html, text, showCopied);
    }
  }

  // Legacy clipboard via hidden contenteditable + execCommand (works for HTML table paste)
  function legacyCopy(html, text, onDone) {
    try {
          ...
          {isSelected&&(
            <td style={{whiteSpace:"nowrap"}}>
              <div style={{display:"flex",gap:3,alignItems:"center"}}>
                <button className="btn" style={{fontSize:10,padding:"2px 7px"}} title="Move up" onClick={e=>{e.stopPropagation();moveProductInCat(p.sku,-1);}}>↑</button>
                <button className="btn" style={{fontSize:10,padding:"2px 7px"}} title="Move down" onClick={e=>{e.stopPropagation();moveProductInCat(p.sku,1);}}>↓</button>
                <button className="btn" style={{fontSize:10,padding:"2px 8px",color:copiedSku===p.sku?"#5a9e5a":"#4a90d9"}} title="Copy as table for Outlook" onClick={e=>{e.stopPropagation();copyProductTable(p);}}>{copiedSku===p.sku?"✓ Copied":"📋 Copy"}</button>
                <button className="btn" style={{fontSize:10,padding:"2px 8px"}} onClick={e=>{e.stopPropagation();setEditing({...p,_origSku:p.sku});}}>✏ Edit</button>
                <button className="btn-del" style={{fontSize:10,padding:"2px 7px"}} onClick={e=>{e.stopPropagation();setProducts(ps=>ps.filter(r=>r.sku!==p.sku));}}>✕ Del</button>
              </div>
            </td>
          )}
        </tr>;
  ...
  const tableHeaders = (
    <tr>
      ...
      <Th l="SKU" f="sku" style={{width:110,whiteSpace:"nowrap"}}/>
      <Th l="Product" f="product" style={{width:140,whiteSpace:"nowrap"}}/>
      <Th l="Description" f="description" style={{minWidth:280}}/>
      <Th l="Pkg" f="pkg" style={{textAlign:"right",width:50}}/>
      <Th l="Pallet" f="pallet" style={{textAlign:"right",width:55}}/>
      <Th l="Price/Unit" f="price" style={{textAlign:"right",width:80}}/>
      <Th l="Pallet $" f="palletPrice" style={{textAlign:"right",width:75}}/>
      <Th l="Prepaid" f="prepaid" style={{textAlign:"right",width:75}}/>
      <Th l="Pre.Pallet" f="prepaidPallet" style={{textAlign:"right",width:80}}/>
      <Th l="Trk Qty" f="truckQty" style={{textAlign:"right",width:65}}/>
      <Th l="Truck $" f="truckPrice" style={{textAlign:"right",width:70}}/>
    </tr>
  );
  ...
  return (
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:T.bg}}>
      ...
      {addModal && (
        <div className="modal-overlay" onClick={()=>setAddModal(false)}>
          <div className="modal" style={{maxWidth:560,width:"94%"}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:10,textTransform:"uppercase",letterSpacing:".12em",color:T.tableHeadText,marginBottom:16}}>＋ Add New Product</div>
            {[{k:"sku",label:"SKU *",ph:"e.g. BMP-NEWSKU01"},{k:"product",label:"Product Name",ph:"e.g. Standard ECB"},{k:"description",label:"Description",ph:"e.g. Std-Single Net Straw Blanket - 8' x 112.5'"}].map(({k,label,ph})=>(
              <div key={k} style={{marginBottom:10}}>
                <div style={{fontSize:9,color:T.muted,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>{label}</div>
                <input value={newProduct[k]} onChange={e=>setNewProduct(p=>({...p,[k]:e.target.value}))} placeholder={ph} style={{width:"100%",fontSize:12,height:32}}/>
              </div>
            ))}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
              {[{k:"pkg",l:"Pkg Qty"},{k:"pallet",l:"Pallet Qty"},{k:"truckQty",l:"Truck Qty"}].map(({k,l})=>(
                <div key={k}><div style={{fontSize:9,color:T.muted,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>{l}</div><input value={newProduct[k]} onChange={e=>setNewProduct(p=>({...p,[k]:e.target.value}))} style={{width:"100%",fontSize:12,height:32}}/></div>
              ))}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
              {[{k:"price",l:"Price/Unit"},{k:"palletPrice",l:"Pallet Price"},{k:"prepaid",l:"Prepaid Unit"},{k:"prepaidPallet",l:"Prepaid Pallet"},{k:"truckPrice",l:"Truck Price"}].map(({k,l})=>(
                <div key={k}><div style={{fontSize:9,color:T.muted,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>{l}</div><input type="number" step="0.01" value={newProduct[k]} onChange={e=>setNewProduct(p=>({...p,[k]:e.target.value}))} placeholder="0.00" style={{width:"100%",fontSize:12,height:32}}/></div>
              ))}
            </div>
            <div style={{marginBottom:16}}>
              <div style={{fontSize:9,color:T.muted,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>Category (optional)</div>
              <input value={newProduct._category||""} onChange={e=>setNewProduct(p=>({...p,_category:e.target.value}))}
                list="cat-datalist-add" placeholder="e.g. Erosion Control Blanket" style={{width:"100%",fontSize:12,height:32}}/>
              <datalist id="cat-datalist-add">{allCategories.filter(c=>c!=="Uncategorized").map(c=><option key={c} value={c}/>)}</datalist>
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
              <button className="btn" onClick={()=>setAddModal(false)}>Cancel</button>
              <button className="btn-gold" disabled={!newProduct.sku.trim()} onClick={()=>{
                if (!newProduct.sku.trim()) return;
                const {_category,...prod} = newProduct;
                const pp = v=>{const n=parseFloat(String(v||"").replace(/[$,\s]/g,""));return isNaN(n)?"":n;};
                const cleaned={...prod,price:pp(prod.price),palletPrice:pp(prod.palletPrice),prepaid:pp(prod.prepaid),prepaidPallet:pp(prod.prepaidPallet),truckPrice:pp(prod.truckPrice)};
                setProducts(prev=>{
                  if((prev||[]).find(p=>p.sku===cleaned.sku)){alert(`SKU "${cleaned.sku}" already exists.`);return prev;}
                  return [...(prev||[]),cleaned];
                });
                if(_category?.trim()) setCategories(prev=>({...prev,[cleaned.sku]:_category.trim()}));
                setNewProduct({sku:"",product:"",description:"",pkg:"",pallet:"",truckQty:"",price:"",palletPrice:"",prepaid:"",prepaidPallet:"",truckPrice:""});
                setAddModal(false);
              }}>Add Product</button>
            </div>
          </div>
      ...
      <div style={{padding:"10px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10,background:T.panelBg||T.headerBg,flexWrap:"wrap"}}>
        ...
        <button className="btn-gold" style={{fontSize:10,padding:"4px 12px",whiteSpace:"nowrap"}} onClick={()=>setAddModal(true)}>＋ Add Product</button>
        ...
        <button className="btn" style={{fontSize:10,padding:"4px 10px",whiteSpace:"nowrap"}} onClick={()=>{
          const headers = ["SKU","Product","Description","Category","Sub-Category","Pkg","Pallet","Truck Qty","Price/Unit","Pallet Price","Prepaid","Prepaid Pallet","Truck Price"];
          const rows = products.map(p=>[
            p.sku, p.product, p.description,
            (categories||{})[p.sku]||"",
            (subCategories||{})[p.sku]||"",
            p.pkg||"", p.pallet||"", p.truckQty||"",
            p.price||"", p.palletPrice||"", p.prepaid||"", p.prepaidPallet||"", p.truckPrice||""
          ].map(v=>`"${String(v).replace(/"/g,'""')}"`).join(","));
          const csv = [headers.join(","), ...rows].join("\n");
          const a = document.createElement("a");
          a.href = URL.createObjectURL(new Blob([csv],{type:"text/csv"}));
          a.download = `BMP-Products-${currency}-${new Date().toISOString().slice(0,10)}.csv`;
          a.click();
        }}>⬇ Export CSV</button>
        <button className="btn-gold" style={{fontSize:10,padding:"4px 12px",whiteSpace:"nowrap"}} onClick={()=>setPriceListModal(true)}>📄 Price List PDF</button>
      </div>

      {/* Category + Sub-category filter — dropdowns */}
      <div style={{padding:"6px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",gap:8,alignItems:"center",background:T.tableHead,flexWrap:"wrap"}}>
        <span style={{fontSize:9,color:T.muted,letterSpacing:".08em",textTransform:"uppercase"}}>Filter:</span>
        <select value={filterCat} onChange={e=>{setFilterCat(e.target.value);setFilterSub("All");}}
          style={{fontSize:11,height:26,padding:"0 6px",background:T.panelBg||T.bg,color:T.subtext,border:`1px solid ${T.border}`,cursor:"pointer",minWidth:180}}>
        ...
        {availableSubCats.length>0&&(
          <select value={filterSub} onChange={e=>setFilterSub(e.target.value)}
            style={{fontSize:11,height:26,padding:"0 6px",background:T.panelBg||T.bg,color:T.accent,border:`1px solid ${T.border}`,cursor:"pointer",minWidth:160}}>
        ...
        {(filterCat!=="All"||filterSub!=="All")&&(
          <button className="btn" style={{fontSize:10,padding:"2px 10px"}} onClick={()=>{setFilterCat("All");setFilterSub("All");}}>Clear ✕</button>
        )}
      </div>
          ...
          <tbody>
            {viewMode==="flat"
              ...
              : allCategories.map(cat=>{
                  ...
                  const isCollapsed = collapsed.has("__all__") ? !collapsed.has(`__open__${cat}`) : collapsed.has(cat);
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          ...
                          {cat!=="Uncategorized"&&(
                            <div style={{marginLeft:"auto",display:"flex",gap:6,alignItems:"center"}} onClick={e=>e.stopPropagation()}>
                              {/* Category reorder */}
                              <button className="btn" style={{fontSize:9,padding:"1px 6px"}} title="Move category up"
                                onClick={e=>{e.stopPropagation();moveCategory(cat,-1);}}>↑</button>
                              <button className="btn" style={{fontSize:9,padding:"1px 6px"}} title="Move category down"
                                onClick={e=>{e.stopPropagation();moveCategory(cat,1);}}>↓</button>
                        ...
                        <tr key={`sub-${cat}-${sub}`}
                          ...
                          <td colSpan={12} style={{padding:"5px 12px 5px 28px",userSelect:"none"}}>
                            <div style={{display:"flex",alignItems:"center",gap:6}}>
                              ...
                              {/* Assign selected products in this cat to this sub-cat */}
                              {selected.size>0&&[...selected].some(s=>(categories||{})[s]===cat)&&(
                                <button className="btn-gold" style={{fontSize:9,padding:"1px 8px",marginLeft:6}}
                                  onClick={e=>{
                                    e.stopPropagation();
                                    const skusInCat=[...selected].filter(s=>(categories||{})[s]===cat);
                                    skusInCat.forEach(s=>assignSubCategory(s,sub));
                                    setSelected(new Set());
                                  }}>
                                  ＋ Assign {[...selected].filter(s=>(categories||{})[s]===cat).length} selected
                                </button>
                              )}
                              <div style={{marginLeft:"auto",display:"flex",gap:3}} onClick={e=>e.stopPropagation()}>
                                {/* Up/Down reorder */}
                                <button className="btn" style={{fontSize:10,padding:"0px 6px",lineHeight:"18px"}}
                                  title="Move up" onClick={e=>{e.stopPropagation();moveSubCat(cat,sub,-1);}}>↑</button>
                                <button className="btn" style={{fontSize:10,padding:"0px 6px",lineHeight:"18px"}}
                                  title="Move down" onClick={e=>{e.stopPropagation();moveSubCat(cat,sub,1);}}>↓</button>
                                <button className="btn-del" style={{fontSize:9,padding:"1px 8px"}}
...
// ─── Price List Modal ──────────────────────────────────────────────────────────
function PriceListModal({products, categories, subCategories, subCatDefs, currency, onClose, T}) {
  const [version,       setVersion]       = useState("V2");
  const [effectiveDate, setEffectiveDate] = useState(()=>{
    const d = new Date();
    return `${d.toLocaleString('default',{month:'long'})} ${d.getDate()}, ${d.getFullYear()}`;
  });
  const [excludedSkus,  setExcludedSkus]  = useState(new Set());
  const [excludeSearch, setExcludeSearch] = useState("");
  const [showExclude,   setShowExclude]   = useState(false);
  const iframeRef = useRef(null);

  function toggleExclude(sku) {
    setExcludedSkus(prev=>{ const n=new Set(prev); n.has(sku)?n.delete(sku):n.add(sku); return n; });
  }
  ...
  // Build category structure using the SAME order shown in the Products tab
  function buildStructure() {
    const assigned = new Set(Object.values(categories||{}).filter(Boolean));
    const savedOrder = (subCatDefs||{}).__catOrder__;
    const baseOrder = Array.isArray(savedOrder) && savedOrder.length ? savedOrder : CATEGORY_ORDER;
    const ordered = baseOrder.filter(c=>assigned.has(c));
    const extras  = Array.from(assigned).filter(c=>!baseOrder.includes(c)&&c!=="Uncategorized").sort();
    const finalCats = [...ordered,...extras,"Uncategorized"];
    return finalCats.map(cat=>{
      const catProds = products.filter(p=>((categories||{})[p.sku]||"Uncategorized")===cat&&!excludedSkus.has(p.sku));
      if (!catProds.length) return null;
      const subs = (cat==="__catOrder__") ? [] : (subCatDefs?.[cat]||[]);
      if (!subs.length) return {cat,prods:catProds,subs:null};
      const subGroups=[];
      subs.forEach(sub=>{
        const sp=catProds.filter(p=>(subCategories||{})[p.sku]===sub);
        if(sp.length) subGroups.push({sub,prods:sp});
      });
      const unassigned=catProds.filter(p=>!(subCategories||{})[p.sku]);
      if(unassigned.length) subGroups.push({sub:null,prods:unassigned});
      return {cat,prods:null,subs:subGroups};
    }).filter(Boolean);
  }

  function usedCols(prods) {
    return {
      pkg:          prods.some(p=>p.pallet||p.pkg),
      price:        prods.some(p=>fmtP(p.price)),
      palletPrice:  prods.some(p=>fmtP(p.palletPrice)),
      prepaid:      prods.some(p=>fmtP(p.prepaid)),
      prepaidPallet:prods.some(p=>fmtP(p.prepaidPallet)),
      truckPrice:   prods.some(p=>fmtP(p.truckPrice)),
    };
  }

  function buildProductTable(prods) {
    const c = usedCols(prods);
    const th = (t,w="")=>`<th style="padding:6px 8px;text-align:center;font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#fff;font-weight:700;border:1px solid #2a2a2a;white-space:nowrap${w?`;width:${w}`:""}">${t}</th>`;
    const headers = [
      `<th style="padding:6px 8px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#fff;font-weight:700;border:1px solid #2a2a2a;width:110px">SKU</th>`,
      `<th style="padding:6px 8px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#fff;font-weight:700;border:1px solid #2a2a2a">Product Description</th>`,
      ...(c.pkg?[th("Pallet Qty","70px")]:[]),
      ...(c.price?[th("Price/Per","85px")]:[]),
      ...(c.palletPrice?[th("Pallet Price","85px")]:[]),
      ...(c.prepaid?[th("Prepaid/Per","85px")]:[]),
      ...(c.prepaidPallet?[th("Pre. Pallet","85px")]:[]),
      ...(c.truckPrice?[th("Truck Price","85px")]:[]),
    ].join("");

    const tdBase = `font-size:10px;padding:5px 8px;border:1px solid #ddd;vertical-align:middle`;
    const rows = prods.map((p,i)=>{
      const bg = i%2===0?"#fff":"#f7f7f5";
      return `<tr style="background:${bg}">
        <td style="${tdBase};font-family:monospace;font-size:9.5px;color:#555">${p.sku||""}</td>
        <td style="${tdBase}">${p.description||p.product||""}</td>
        ${c.pkg?`<td style="${tdBase};text-align:center">${p.pallet||p.pkg||"—"}</td>`:""}
        ${c.price?`<td style="${tdBase};text-align:right">${fmtP(p.price)||"—"}</td>`:""}
        ${c.palletPrice?`<td style="${tdBase};text-align:right">${fmtP(p.palletPrice)||"—"}</td>`:""}
        ${c.prepaid?`<td style="${tdBase};text-align:right">${fmtP(p.prepaid)||"—"}</td>`:""}
        ${c.prepaidPallet?`<td style="${tdBase};text-align:right">${fmtP(p.prepaidPallet)||"—"}</td>`:""}
        ${c.truckPrice?`<td style="${tdBase};text-align:right">${fmtP(p.truckPrice)||"—"}</td>`:""}
      </tr>`;
    }).join("");

    return `<table style="width:100%;border-collapse:collapse;margin-bottom:0">
      <thead><tr style="background:#1a1a1a">${headers}</tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
  }

  // Dedicated first page: loyalty note + spec link + effective date + per-category notes
  function buildNotesPage() {
    const noteBox = (title, items) => `
      <div style="border:1px solid #ccc;margin-bottom:10px;break-inside:avoid">
        <div style="background:#1a1a1a;color:#fff;padding:6px 12px;font-size:12px;font-weight:700;letter-spacing:.03em">${title}</div>
        <div style="padding:8px 14px">
          <ul style="margin:0;padding-left:18px;font-size:11px;color:#333;line-height:1.7">
    ...
    return `
    <div style="page-break-after:always;min-height:250mm;display:flex;flex-direction:column">
      <!-- Header matches the Quote PDF top-left -->
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8mm;padding-bottom:5mm;border-bottom:2px solid #1a1a1a">
        <div>
          <img src="${FAVICON_DARK}" style="height:54px;width:auto;mix-blend-mode:multiply" alt="BMP Supplies"/>
        ...
        <div style="text-align:right">
          <div style="font-size:34px;font-weight:700;color:#1a1a1a;line-height:1">Price List</div>
          <div style="font-size:16px;color:#666;margin-top:2px">${new Date().getFullYear()}</div>
          <div style="font-size:11px;color:#888;margin-top:8px">${version}</div>
          <div style="font-size:11px;color:#888">Effective ${effectiveDate}</div>
        </div>
      </div>

      <!-- Loyalty message -->
      <div style="background:#f7f7f5;border-left:3px solid #1a1a1a;padding:12px 16px;margin-bottom:14px">
        <div style="font-size:12px;color:#333;line-height:1.7">
          Thank you for being a valued BMP Supplies distributor. We appreciate your continued partnership.
          <strong>This price list is intended for your organization only — please do not share it outside your organization.</strong>
        </div>
        <div style="font-size:11px;color:#555;margin-top:8px">
          All product spec sheets are available at our Download Centre:
          <a href="https://www.bmpsupplies.com/download-centre" style="color:#1a5fa8;text-decoration:underline">https://www.bmpsupplies.com/download-centre</a>
        </div>
      </div>

      ${noteBox("Catch Basin Notes", [
        "Minimum orders of $1,000",
        "Prices are per individual item",
        "No split packages",
      ])}

      ${noteBox("Turbidity Curtain Notes", [
        "Custom lengths, depths, and materials available, made to specification. For more than one pallet quotes please contact sales@bmpsupplies.com",
        `Turbidity Curtain Selector tool: <a href="https://www.bmpsupplies.com/turbidity-curtains-selector-tool" style="color:#1a5fa8;text-decoration:underline">https://www.bmpsupplies.com/turbidity-curtains-selector-tool</a>`,
        `Compare Turbidity Curtain Specs: <a href="https://www.bmpsupplies.com/turbidity-curtains-compare" style="color:#1a5fa8;text-decoration:underline">https://www.bmpsupplies.com/turbidity-curtains-compare</a>`,
      ])}

      ${noteBox("Erosion Control Blankets", [
        "This pricing is for single FTL or LTL orders only. For multiple truck loads please reach out to sales@bmpsupplies.com",
        "To receive FTL pricing at least half the truck must be filled with Erosion Control Blankets",
      ])}

      ${noteBox("Truckload (FTL) Quantities", [
        "ECB's — 8' Wide Rolls — 24 Pallets per truckload (600 rolls total)",
        "Wattles — 9\" Diam Logs — 26 Pallets per truckload (364 logs total)",
      ])}
    </div>`;
  }
  ...
  function buildHTML() {
    ...
    let catSections = "";
    structure.forEach(({cat,prods,subs})=>{
      catSections += `<div style="margin-bottom:18px;page-break-inside:avoid">
        <div style="background:#1a1a1a;color:#fff;text-align:center;padding:8px 12px;font-size:14px;font-weight:700;letter-spacing:.04em">${cat}</div>`;
      if (prods) {
        catSections += buildProductTable(prods);
      } else {
        subs.forEach(({sub,prods:sp})=>{
          if (sub) catSections += `<div style="background:#444;color:#fff;padding:5px 10px;font-size:11px;font-weight:700;letter-spacing:.03em">${sub}</div>`;
          catSections += buildProductTable(sp);
        });
      }
      catSections += `</div>`;
    });

    const footerText = `BMP Supplies&nbsp;&nbsp;|&nbsp;&nbsp;Price List ${new Date().getFullYear()} - ${version}&nbsp;&nbsp;|&nbsp;&nbsp;bmpsupplies.com`;

    return `<!DOCTYPE html><html><head><meta charset="UTF-8"/>
<title>BMP Supplies Price List ${version}</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box;}
  html,body{background:#fff;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:#1a1a1a;font-size:11px;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
  @page{size:8.5in 11in;margin:12mm 12mm 16mm 12mm;}
  @media print{
    .no-print{display:none!important;}
    .print-footer{position:fixed;bottom:6mm;left:0;right:0;text-align:center;font-size:8px;color:#999;}
    .page-counter::after{counter-increment:page;content:" | Page " counter(page);}
  }
  a{color:#1a5fa8;}
</style></head><body>
<div class="no-print" style="position:fixed;top:0;left:0;right:0;background:#1a1a1a;padding:8px 16px;display:flex;gap:10px;z-index:999">
  <button onclick="window.print()" style="background:#c8a96e;color:#0a0a0a;border:none;padding:8px 20px;font-size:12px;font-weight:700;cursor:pointer">🖨 Print / Save as PDF</button>
  <button onclick="window.close()" style="background:#333;color:#ccc;border:none;padding:8px 14px;font-size:12px;cursor:pointer">Close</button>
</div>
<div style="padding-top:44px">
  ${buildNotesPage()}
  ${catSections}
  <div style="margin-top:10mm;padding-top:3mm;border-top:1px solid #ddd;text-align:center;font-size:8px;color:#aaa">
    Prices subject to change without notice &nbsp;|&nbsp; All prices in ${currency} &nbsp;|&nbsp; Tax not included &nbsp;|&nbsp; sales@bmpsupplies.com &nbsp;|&nbsp; 604.542.0222
  </div>
</div>
<div class="print-footer page-counter">${footerText}</div>
</body></html>`;
  }
  ...
  },[version,effectiveDate,excludedSkus]);

  function doPrint() {
    const w = window.open('',`BMP Price List ${version}`,'width=900,height=700');
    if (w) {
      w.document.open(); w.document.write(buildHTML()); w.document.close();
      w.document.title = `BMP Supplies Price List ${version} - ${effectiveDate}`;
      setTimeout(()=>{ try{w.focus();w.print();}catch(e){} },800);
    }
  }

  const filteredProds = excludeSearch
    ? products.filter(p=>(p.sku||"").toLowerCase().includes(excludeSearch.toLowerCase())||(p.product||"").toLowerCase().includes(excludeSearch.toLowerCase()))
    : products;
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",zIndex:300,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:"#111",borderBottom:"1px solid #222",padding:"10px 16px",display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
        <div style={{fontSize:11,color:"#c8a96e",letterSpacing:".1em",textTransform:"uppercase",fontWeight:600,flex:1}}>📄 Price List PDF</div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <label style={{fontSize:10,color:"#888"}}>Version:</label>
          <input value={version} onChange={e=>setVersion(e.target.value)}
            style={{width:60,fontSize:12,padding:"3px 6px",background:"#1e1e1e",border:"1px solid #333",color:"#e8e8e8",textAlign:"center"}}/>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <label style={{fontSize:10,color:"#888"}}>Effective date:</label>
          <input value={effectiveDate} onChange={e=>setEffectiveDate(e.target.value)}
            style={{width:170,fontSize:12,padding:"3px 6px",background:"#1e1e1e",border:"1px solid #333",color:"#e8e8e8"}}/>
        </div>
        <button onClick={()=>setShowExclude(s=>!s)}
          style={{background:showExclude?"#c8a96e":"transparent",border:"1px solid #555",color:showExclude?"#0a0a0a":"#ccc",padding:"5px 12px",fontSize:10,cursor:"pointer"}}>
          {excludedSkus.size>0?`Exclude (${excludedSkus.size} hidden)`:"Exclude Products"}
        </button>
        <button onClick={doPrint}
          style={{background:"#c8a96e",border:"none",color:"#0a0a0a",padding:"7px 18px",fontSize:11,fontWeight:700,cursor:"pointer",letterSpacing:".06em"}}>
          🖨 Print / Save PDF
        </button>
        <button onClick={onClose}
          style={{background:"transparent",border:"1px solid #333",color:"#666",padding:"7px 12px",fontSize:11,cursor:"pointer"}}>✕</button>
      </div>

      {/* Exclude panel */}
      {showExclude&&(
        <div style={{background:"#0d0d0d",borderBottom:"1px solid #222",padding:"10px 16px",flexShrink:0,maxHeight:200,overflow:"hidden",display:"flex",flexDirection:"column",gap:8}}>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <input placeholder="Search SKU or product…" value={excludeSearch} onChange={e=>setExcludeSearch(e.target.value)}
              style={{flex:1,maxWidth:280,fontSize:11,height:26,background:"#1a1a1a",border:"1px solid #333",color:"#e8e8e8",padding:"0 8px"}}/>
            <span style={{fontSize:10,color:"#555"}}>{excludedSkus.size} excluded</span>
            {excludedSkus.size>0&&<button className="btn" style={{fontSize:9,padding:"2px 8px"}} onClick={()=>setExcludedSkus(new Set())}>Clear all</button>}
          </div>
          <div style={{overflowY:"auto",display:"flex",flexWrap:"wrap",gap:4}}>
            {filteredProds.slice(0,150).map(p=>(
              <label key={p.sku} style={{display:"flex",alignItems:"center",gap:4,fontSize:10,color:excludedSkus.has(p.sku)?"#e8472c":"#888",cursor:"pointer",background:"#1a1a1a",padding:"2px 8px",borderRadius:2,border:`1px solid ${excludedSkus.has(p.sku)?"#e8472c":"#222"}`}}>
                <input type="checkbox" checked={excludedSkus.has(p.sku)} onChange={()=>toggleExclude(p.sku)} style={{margin:0,width:11,height:11}}/>
      ...
      {/* Preview */}
      <div style={{flex:1,overflow:"hidden",background:"#2a2a2a",display:"flex",justifyContent:"center",padding:"16px"}}>
        <iframe ref={iframeRef} style={{width:"215.9mm",height:"100%",border:"none",background:"#fff",boxShadow:"0 4px 32px rgba(0,0,0,.5)"}}/>
      </div>
    </div>
  );
}
...
function QuoteGroupedList({quotes, activeQuote, setActiveQuote, setDeleteConfirm, duplicateQuote, searchQ, quoteSort, T}) {
  ...
  const {groups, thisWeekKey, thisMonthKey} = useMemo(() => {
    ...
    const groups = [];
    // Always show This Week folder, even if empty (shows placeholder)
    groups.push({id:'thisweek', label:'This Week', quotes:thisWeek, defaultOpen:true, isThisWeek:true});
    monthKeys.forEach(mk => {
      const label = mk==='unsorted' ? 'Unsorted' : getMonthLabel(mk);
      // Close all month folders when This Week is shown (it always is now)
      groups.push({id:mk, label, quotes:byMonth[mk], defaultOpen:false});
    });
  ...
  if (filtered.length === 0 && (quotes||[]).length > 0) {
    return <div style={{padding:16, color:T.muted, fontSize:11, textAlign:'center'}}>No quotes match your search</div>;
  }

  // If no quotes at all, still render the This Week folder with empty state
  if ((quotes||[]).length === 0) {
    return (
      <div style={{paddingBottom:8}}>
        <div style={{display:'flex',alignItems:'center',gap:6,padding:'6px 12px',background:T.tableHead,borderBottom:`1px solid ${T.border}`,borderTop:`1px solid ${T.border}`,userSelect:'none',position:'sticky',top:0,zIndex:2}}>
          <span style={{fontSize:10,color:T.muted,width:10,flexShrink:0}}>▾</span>
          <span style={{fontSize:10,fontWeight:600,color:T.subtext,letterSpacing:'.04em',flex:1}}>This Week</span>
          <span style={{fontSize:9,background:T.borderMid,color:T.muted,borderRadius:10,padding:'1px 7px',fontWeight:600,flexShrink:0}}>0</span>
        </div>
        <div style={{padding:'14px 12px',textAlign:'center',color:T.muted,fontSize:11,fontStyle:'italic',borderBottom:`1px solid ${T.border}`}}>
          No quotes made this week yet
        </div>
      </div>
    );
  }
    ...
    <div style={{paddingBottom:8}}>
      {groups.map(group => (
        <div key={group.id}>
          ...
          {/* Quotes in folder */}
          {open[group.id] && group.isThisWeek && group.quotes.length === 0 && (
            <div style={{padding:'14px 12px', textAlign:'center', color:T.muted, fontSize:11, fontStyle:'italic', borderBottom:`1px solid ${T.border}`}}>
              No quotes made this week yet
            </div>
          )}
...
function PipelineTab({quotes, setQuotes, T, loginName, setActiveQuote, setActiveTab, leads, setLeads}) {
  ...
  const [activeRep, setActiveRep] = useState(()=>repsWithQuotes[0]||"");
  const [followUpOpen, setFollowUpOpen] = useState({}); // quoteId -> bool
  const [followUpText, setFollowUpText] = useState({});  // quoteId -> text
  ...
  useEffect(()=>{
    if (!repsWithQuotes.includes(activeRep) && repsWithQuotes.length>0)
      setActiveRep(repsWithQuotes[0]);
  }, [repsWithQuotes.join(',')]);

  function toggleSection(k){setOpenSections(s=>({...s,[k]:!s[k]}));}

  function daysSince(savedDate){
  ...
  function daysUntilExpiry(q){
    ...
    const d = new Date(q.savedDate);
    if(isNaN(d)) return null;
    const expiry = new Date(d.getTime() + (parseInt(q.validFor)||30)*24*60*60*1000);
    ...
    setQuotes(prev => {
      ...
      if(updated) {
        import('firebase/firestore').then(({doc, setDoc, getFirestore})=>{
          setDoc(doc(getFirestore(),'quotes',String(id)), updated).catch(console.error);
        }).catch(()=>{});
      }
      return next;
    });
  ...
  function addFollowUp(q){
    const note = (followUpText[q.id]||"").trim();
    const today = new Date().toLocaleDateString("en-CA",{month:"short",day:"numeric",year:"numeric"});
    const entry = {date:today, by:loginName, note};
    const newFollowUps = [...(q.followUps||[]), entry];
    updateQuote(q.id, {followUps:newFollowUps, quoteStatus:'inprogress'});
    setFollowUpOpen(s=>({...s,[q.id]:false}));
    setFollowUpText(s=>({...s,[q.id]:""}));
    // Sync lastContact + status to matching lead
    if (setLeads) {
      ...
      setLeads(prev => prev.map(l => {
        ...
        if (matchByNum || matchByComp) return {...l, lastContact: today, status:'inprogress'};
        return l;
      }));
    }
  }
  ...
  function QuoteCard({q}){
    ...
    const total = (q.lineItems||[]).reduce((s,li)=>(parseFloat(li.unitPrice)||0)*(parseInt(li.qty)||0)+s,0);
    const followUps = q.followUps||[];
    const isOpen = followUpOpen[q.id];
    const status = q.quoteStatus||'inprogress';

    return (
      <div style={{background:T.cardBg, border:`1px solid ${T.border}`, borderRadius:4, padding:'12px 14px', marginBottom:8}}>
        {/* Header row */}
        <div style={{display:'flex', alignItems:'flex-start', gap:10, marginBottom:8}}>
          <div style={{flex:1, minWidth:0}}>
            <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:3, flexWrap:'wrap'}}>
              <span style={{fontSize:13, fontWeight:700, color:'#c8a96e'}}>{q.quoteNum}</span>
              <span style={{fontSize:11, color:T.subtext, fontWeight:600}}>{q.company||q.name||'—'}</span>
              {q.company&&q.name&&<span style={{fontSize:11, color:T.muted}}>{q.name}</span>}
              {total>0&&<span style={{fontSize:11, color:T.accent, fontWeight:600, marginLeft:'auto'}}>{fmtCur(total)} {q.currency}</span>}
            </div>
            <div style={{display:'flex', gap:10, flexWrap:'wrap'}}>
              <span style={{fontSize:10, color:T.muted}}>{days}d ago · saved {q.savedDate}</span>
              {expDays!==null&&expDays<=14&&(
                <span style={{fontSize:10, fontWeight:600, color:expDays<=3?'#e8472c':expDays<=7?'#f5a623':'#f5a623'}}>
                  {expDays<=0?'EXPIRED':`Expires in ${expDays}d`}
                </span>
              )}
              {followUps.length>0&&(
                <span style={{fontSize:10, color:'#4a90d9'}}>📞 {followUps.length}× followed up</span>
              )}
            </div>
          </div>
          {/* Open quote button */}
          <button onClick={()=>{setActiveQuote(q);setActiveTab('quotes');}}
            style={{background:'transparent',border:`1px solid ${T.border}`,color:T.muted,padding:'3px 10px',fontSize:10,cursor:'pointer',borderRadius:2,flexShrink:0,whiteSpace:'nowrap'}}
            onMouseOver={e=>e.currentTarget.style.color=T.accent}
            onMouseOut={e=>e.currentTarget.style.color=T.muted}>
            Open ↗
          </button>
        ...
        {/* Last follow-up note */}
        {followUps.length>0&&(
          <div style={{background:T.tableHead, borderRadius:3, padding:'6px 10px', marginBottom:8, fontSize:11, color:T.muted, borderLeft:`3px solid #4a90d9`}}>
            <span style={{color:T.subtext, fontWeight:600}}>{followUps[followUps.length-1].by} · {followUps[followUps.length-1].date}: </span>
            {followUps[followUps.length-1].note||<em>No note</em>}
          </div>
        )}

        {/* Action row */}
        <div style={{display:'flex', gap:8, alignItems:'center', flexWrap:'wrap'}}>
          <button onClick={()=>setFollowUpOpen(s=>({...s,[q.id]:!s[q.id]}))}
            style={{background:isOpen?'#4a90d9':'transparent', border:`1px solid ${isOpen?'#4a90d9':T.borderMid}`,
              color:isOpen?'#fff':T.subtext, padding:'4px 12px', fontSize:11, cursor:'pointer', borderRadius:2, fontWeight:600}}>
            📞 {isOpen?'Cancel':'+ Follow Up'}
          </button>
          <select value={status} onChange={e=>setStatus(q,e.target.value)}
            style={{fontSize:11, background:T.inputBg, border:`1px solid ${T.borderMid}`, color:T.subtext,
              padding:'4px 8px', borderRadius:2, cursor:'pointer'}}>
            {STATUS_OPTIONS.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}
          </select>
          {(q.notes||q.internalNotes)&&(
            <span style={{fontSize:10, color:T.muted, fontStyle:'italic', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
              📝 {q.internalNotes||q.notes}
            </span>
          )}
        </div>
        ...
        {/* Follow-up form */}
        {isOpen&&(
          <div style={{marginTop:10, background:T.panelBg, border:`1px solid #4a90d9`, borderRadius:3, padding:10}}>
            <div style={{fontSize:10, color:'#4a90d9', fontWeight:600, marginBottom:6, letterSpacing:'.04em'}}>ADD FOLLOW-UP NOTE</div>
            <textarea value={followUpText[q.id]||""}
              onChange={e=>setFollowUpText(s=>({...s,[q.id]:e.target.value}))}
              onMouseDown={e=>e.stopPropagation()}
              onClick={e=>e.stopPropagation()}
              placeholder="What happened? Next steps?"
              style={{width:'100%', height:60, fontSize:12, resize:'vertical', background:T.inputBg,
                border:`1px solid ${T.borderMid}`, color:T.text, padding:'6px 8px', borderRadius:2,
                fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif", lineHeight:1.4, marginBottom:8,
                boxSizing:'border-box', display:'block'}}/>
            <div style={{display:'flex', gap:8}}>
              <button onClick={()=>addFollowUp(q)}
                style={{background:'#4a90d9', border:'none', color:'#fff', padding:'6px 16px', fontSize:12,
                  fontWeight:600, cursor:'pointer', borderRadius:2}}>
                Save Follow-Up
              </button>
              <button onClick={()=>setFollowUpOpen(s=>({...s,[q.id]:false}))}
                style={{background:'transparent', border:`1px solid ${T.borderMid}`, color:T.muted,
        ...
        {followUps.length>1&&(
          <details style={{marginTop:8}}>
            <summary style={{fontSize:10, color:T.muted, cursor:'pointer', userSelect:'none'}}>
              Show all {followUps.length} follow-ups
            </summary>
            <div style={{marginTop:6, display:'flex', flexDirection:'column', gap:4}}>
              {[...followUps].reverse().map((fu,i)=>(
                <div key={i} style={{fontSize:11, color:T.muted, padding:'4px 8px',
                  background:T.tableHead, borderRadius:2, borderLeft:`2px solid #4a90d9`}}>
                  <span style={{color:T.subtext, fontWeight:600}}>{fu.by} · {fu.date}: </span>
                  {fu.note||<em style={{color:T.muted}}>No note</em>}
                </div>
              ))}
            </div>
          </details>
    ...
    <div style={{display:'flex', flexDirection:'column', height:'100%', overflow:'hidden', background:T.bg}}>
      ...
      <div style={{display:'flex', borderBottom:`2px solid ${T.border}`, background:T.headerBg, flexShrink:0, overflowX:'auto'}}>
        {repsWithQuotes.map(rep=>{
          const repQs = savedQuotes.filter(q=>q.savedBy===rep);
          const urgent = repQs.filter(q=>['overdue','followup'].includes(getQuoteBucket(q))).length;
          return (
            <button key={rep} onClick={()=>setActiveRep(rep)}
              style={{padding:'10px 20px', background:'none', border:'none', cursor:'pointer',
                borderBottom:`2px solid ${activeRep===rep?'#c8a96e':'transparent'}`,
                marginBottom:-2, fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif",
                fontSize:13, fontWeight:activeRep===rep?700:400,
                color:activeRep===rep?'#c8a96e':T.subtext,
                display:'flex', alignItems:'center', gap:6, flexShrink:0}}>
              {rep}
              {urgent>0&&<span style={{background:'#e8472c', color:'#fff', borderRadius:10,
                fontSize:9, fontWeight:700, padding:'1px 6px', lineHeight:1.4}}>{urgent}</span>}
            </button>
          );
        })}
      </div>
      ...
      {(()=>{
        ...
        <div style={{flex:1, overflowY:'auto', padding:16}}>
          ...
          return (
            <div key={bucket.key} style={{marginBottom:12}}>
              {/* Section header */}
              <div onClick={()=>toggleSection(bucket.key)}
                style={{display:'flex', alignItems:'center', gap:8, padding:'8px 12px',
                  background:bucket.bg||T.tableHead, border:`1px solid ${T.border}`,
                  borderLeft:`3px solid ${bucket.color}`, borderRadius:3, cursor:'pointer',
                  userSelect:'none', marginBottom:isOpen?8:0}}>
                <span style={{fontSize:11, fontWeight:700, color:bucket.color, flex:1, letterSpacing:'.04em'}}>
                  {bucket.label.toUpperCase()}
                </span>
                <span style={{fontSize:10, background:T.tableHead, color:T.muted,
                  borderRadius:10, padding:'1px 8px', fontWeight:600}}>{bQuotes.length}</span>
                <span style={{fontSize:10, color:T.muted}}>{isOpen?'▾':'▸'}</span>
              </div>
              {isOpen&&bQuotes.map(q=><QuoteCard key={q.id} q={q}/>)}
            </div>
          );
        })}
        {repQuotes.length===0&&(
            ...
            {sorted.map(lead=>{
              ...
              if(isEditing) return (
                <tr key={lead.id} style={{background:T.tableHead}}>
                  {['company','contact','type','leadSource','potentialValue','status','lastContact','quoteNumber','productsQuoted','notes'].map(f=>(
              ...
              return (
                <tr key={lead.id}
                  ...
                  <td style={{...tdStyle,fontFamily:'monospace',fontSize:10,color:T.accent,whiteSpace:'nowrap'}}>{lead.quoteNumber||'—'}</td>
